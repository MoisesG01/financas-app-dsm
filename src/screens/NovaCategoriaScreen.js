...
export default function NovaCategoriaScreen({ navigation, route }) {
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      await categoriaService.criar(nome.trim(), tipo);
      showToast('Categoria criada com sucesso!', 'success');
      setTimeout(() => {
        navigation.goBack();
      }, 1000);
    } catch (error) {
      const errorMsg = error.response?.data?.erro || 'Erro ao criar categoria';
