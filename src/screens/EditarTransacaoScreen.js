...
export default function EditarTransacaoScreen({ navigation, route }) {
  ...
  const carregarTransacao = async () => {
    try {
      ...
      if (transacao.data) {
        try {
          ...
          if (!isNaN(date.getTime())) {
            ...
            dataFormatada = transacao.data.split('T')[0];
          }
        } catch {
          dataFormatada = transacao.data.split('T')[0] || '';
        }
      ...
      const tipoTransacao = transacao.tipo || 'despesa';
      setTipo(tipoTransacao);
      const categoriaId = transacao.id_categoria ? String(transacao.id_categoria) : '';
      setIdCategoria(categoriaId);
      
      // Carregar categorias após definir o tipo
      await carregarCategoriasParaTipo(tipoTransacao, categoriaId);
    } catch (error) {
  ...
  const carregarCategoriasParaTipo = async (tipoAtual, categoriaAtual) => {
    setLoadingCategorias(true);
    try {
      const dados = await categoriaService.listar();
      const filtradas = dados.filter((cat) => cat.tipo === tipoAtual);
      setCategorias(filtradas);
      
      if (filtradas.length > 0) {
        // Se a categoria atual existe na lista filtrada, mantê-la
        const categoriaExiste = filtradas.find(c => String(c.id_categoria) === categoriaAtual);
        if (categoriaExiste && categoriaAtual) {
          setIdCategoria(categoriaAtual);
        } else {
          // Caso contrário, definir a primeira categoria
  ...
  const carregarCategorias = async () => {
    await carregarCategoriasParaTipo(tipo, idCategoria);
  };
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.erro || error.message || 'Erro ao atualizar transação';
  ...
  if (carregando) {
    return (
      <View style={styles.container}>
        <LoadingOverlay visible={true} message="Carregando transação..." />
      </View>
    );
  }
